...
- Map browser interaction is not separately covered by the existing visual
  matrix; it remains a manual follow-up after the visual baseline review.
...
3. Add a dedicated browser test for the explicit `View on map` flow and its
   semantic fallback before calling the optional map capability release-ready.

## Status

**PARTIAL — functional gates are green; visual acceptance and dedicated map
