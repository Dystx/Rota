...
| Playwright global setup against current hosted Supabase | Blocked by fixture collision | Existing trip id `3` belongs to a different account; setup refuses to reassign it. A follow-up fixture change is in progress to create a unique owned trip and update tests dynamically. |
