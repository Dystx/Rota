...
// the same public API. The web app imports `buildWorkerPlan`
// from the `@repo/workers/plan` subpath instead — that path
// resolves to `./plan.ts` and never evaluates the Node-only
// `@repo/ingest` + `duckdb` chain above, which Next.js refuses
// to bundle into the client/edge runtimes. See
// `apps/workers/package.json` exports.
export { buildWorkerPlan, WORKER_EXECUTION_TARGET, type WorkerPlanInput } from "./plan";
...
// `WorkerPlanInput` is re-exported from `./plan` above. The pure
// builder itself lives in `./plan` so the web app can import it
// without pulling in the Node-only `@repo/ingest` + `duckdb`
// chain in this barrel.
