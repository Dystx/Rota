/**
 * Pure worker-plan builder.
 *
 * Lives in its own module so the web app can import it without
 * pulling in the Node-only workers barrel (which transitively
 * imports `duckdb` via `@repo/ingest` — a native module Next.js
 * refuses to bundle into the client/edge runtimes). The web app
 * consumes this through the `@repo/workers/plan` subpath; the
 * Node workers process imports the same module from the relative
 * path. See `apps/workers/package.json` exports.
 */
    ...
    jobs: [
      ...
      },
      {
        ...
        summary: isPaid
          ...
          : "PDF export remains blocked until the trip is paid.",
