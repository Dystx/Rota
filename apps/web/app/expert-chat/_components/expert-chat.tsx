...
/**
 * ExpertChat — Tier 2 specialist chat (mock).
 *
 * Extracted from the original `/expert-chat/page.tsx` so the
 * page can be a server component that validates `?trip=<id>`
 * before mounting the chat. When the trip param is missing
 * the page renders an `EmptyState` instead of this component.
 *
 * The chat is a 2-pane (timeline + chat) on desktop, single-pane
 * (chat only) on mobile. The chat stream is interactive: typing
 * into the textarea and pressing Enter (or clicking Send) appends
 * a user bubble and a simulated Ana reply from a small
 * canned-response set. The "Specialist Recommendation" card's
 * Decline / Add buttons also dismiss or accept the suggestion.
 *
 * The full triage queue is wired by the Tier 3 backend agent
 * (Phase 9.4); this component is the UX scaffold + local state.
 *
 * TODO: replace the hardcoded Kyoto seed timeline with a
 * `trip.days` prop once the page is fully wired to
 * `getTripDraftById`.
 */
...
export interface ExpertChatProps {
  /** The trip this chat is scoped to. */
  tripId: string;
}
