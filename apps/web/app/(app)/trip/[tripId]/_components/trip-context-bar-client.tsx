      ...
      {sheet ? (
        <div role="dialog" aria-modal="true" aria-label={`Edit ${sheet}`} className="fixed inset-0 z-50 grid place-items-end bg-black/30 p-4" onClick={() => setSheet(null)}>
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <p className="mt-3 text-sm text-on-surface-variant">Context edits are saved from your trip brief. Open the planner to choose a new value.</p>
            <a href="/planner" className="mt-5 inline-flex min-h-11 items-center rounded-full bg-olive-light px-5 text-white">Open planner</a>
          </div>
        </div>
      ) : null}
    </>
