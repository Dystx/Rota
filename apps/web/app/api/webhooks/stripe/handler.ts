...
function toVerificationStatus(result: StripeWebhookVerificationResult): Response {
  ...
  return Response.json({ error: { code: "invalid_webhook", message: "Stripe webhook signature could not be verified." } }, { status });
}
...
export async function handleStripeWebhookRequest(request: Request, dependencies: StripeWebhookDependencies = {}): Promise<Response> {
  ...
  try {
    ...
    if (isWebhookConfigError(error)) return internalError("Stripe webhook secret is not configured.", 503);
    return internalError("Stripe webhook could not be configured.");
  }

  const verification = await verifyWebhook({ now: dependencies.now, payload: rawPayload, secret: webhookSecret, signatureHeader });
  if (!verification.ok) return toVerificationStatus(verification);

  const checkoutSession = readCheckoutSessionCompletedEvent(verification.event);
  if (!checkoutSession) return Response.json({ ignored: true, status: "ignored" });
  ...
  try {
    ...
    if (result.status === "invalid") return validationError("Stripe webhook metadata did not match an existing trip.");
    return Response.json({ eventId: verification.event.id, purchaseKind: fulfillmentInput.purchaseKind, status: result.status, tripId: fulfillmentInput.tripId });
  } catch (error) {
    if (isPersistenceConfigError(error)) return internalError("Persistence is not configured.", 503);
