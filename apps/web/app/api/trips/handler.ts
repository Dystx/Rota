...
import { resolveDefaultAnalyticsProvider, tryCapture, type AnalyticsProvider } from "@repo/analytics";
import { classifyErrorKind, resolveDefaultMonitoringProvider, safeMonitoringRoute, tryCapture as tryCaptureMonitoring, type MonitoringProvider } from "@repo/monitoring";
import { internalError, isApiResponse, requireApiRole, validationError, type AuthorizedApiContext } from "@/lib/auth/api";

const invalidJsonErrors = { rawBrief: ["Request body must be valid JSON."] } satisfies TripBriefValidationErrors;

export type TripCreateDependencies = {
...
export async function handleTripCreateRequest(request: Request, dependencies: TripCreateDependencies = {}) {
  ...
  if (!parsed.success) return validationError("Trip brief validation failed.", parsed.error.flatten().fieldErrors);

  try {
    const result = await createDraft(parsed.data, { actor: auth.actor, ownerUserId: auth.userId });
    ...
    return Response.json({ message: "Trip brief saved.", ...result }, { status: 201 });
  } catch (error) {
    ...
    await tryCaptureMonitoring(monitor, {
      ...
      severity: "error",
      surface: "api",
      properties: { route: safeMonitoringRoute("/api/trips"), method: "POST", status, errorCode, errorKind: classifyErrorKind(error) }
    });
