...
type PersonaSpec = { email: string; fileName: string; passwordEnv: string; role: PersonaRole; displayName: string };
type CookieRecord = {
  domain: string;
  expires: number;
  httpOnly: boolean;
...
const PERSONAS: readonly PersonaSpec[] = [
  { email: "e2e-admin@rota.test", fileName: "admin.json", passwordEnv: "E2E_TEST_USER_PASSWORD", role: "admin", displayName: "E2E Admin" },
  { email: "e2e-reviewer@rota.test", fileName: "reviewer.json", passwordEnv: "E2E_TEST_USER_PASSWORD", role: "reviewer", displayName: "E2E Reviewer" },
  { email: "e2e-traveler@rota.test", fileName: "traveler.json", passwordEnv: "E2E_TEST_USER_PASSWORD", role: "traveler", displayName: "E2E Traveler" }
];
...
async function upsertPersona(owner: pg.Pool, persona: PersonaSpec, password: string): Promise<string> {
  const existing = await owner.query<{ id: string }>("select id from authn.user where lower(email) = lower($1) limit 1", [persona.email]);
  ...
  if (persona.role === "reviewer") {
    await owner.query(
      `insert into app.reviewers (id, user_id, name, country, regions, languages, specialties, status, rating, bio, response_promise)
       values ('e2e-reviewer', $1, $2, 'Portugal', '{lisbon,douro-valley}', '{pt,en}', '{Pacing,Food}', 'Active', 5, 'Playwright-owned reviewer fixture.', 'Within one business day')
       on conflict (id) do update set user_id = excluded.user_id, name = excluded.name, status = excluded.status, updated_at = now()`,
...
async function ensureTravelerTrip(owner: pg.Pool, ownerUserId: string): Promise<{ tripId: string; tripBriefId: string; ownerUserId: string }> {
  const title = "Playwright-owned Portugal route [e2e-fixture]";
  const existing = await owner.query<{ id: string; trip_brief_id: string }>("select id, trip_brief_id from app.trips where owner_user_id = $1 and title = $2 limit 1", [ownerUserId, title]);
  if (existing.rows[0]) return { ownerUserId, tripBriefId: existing.rows[0].trip_brief_id, tripId: existing.rows[0].id };
  ...
  await owner.query(
    `insert into app.trip_briefs
     ...
     values ($1, $2, 'portugal', '{lisbon,douro-valley}', '2027-04-10', '2027-04-15', 5, 2, 'couple', 'mid-range', 'calm', '{local-food,old-streets,sea-views}', '{casual-local-meals}', '{rushed-schedules}', 'train-and-transfers', 'Lisbon historic center', 'Playwright-owned traveler fixture [e2e-fixture]', $3, 'submitted')`,
    [briefId, ownerUserId, JSON.stringify(normalized)]
  );
...
export default async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    for (const persona of PERSONAS) {
      ...
      userIds.set(persona.role, userId);
    ...
    const travelerUserId = userIds.get("traveler");
